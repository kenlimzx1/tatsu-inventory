...
@ccclass('StatusPointsText')
export class StatusPointsText extends Component {
  @property(Label)
  ...
  public setAmountAndMaxAmount(amount: number, maxAmount: number) {
    this.currentAmount = amount;
    this.targetAmount = amount;
    this.maxAmount = maxAmount;
    this.statusLabel.string = `${this.formatAmountString(amount)} / ${this.formatAmountString(this.maxAmount)}`;
  }

  public updateText(endAmount: number, onComplete: (() => void) | null = null) {
    this.unscheduleAllCallbacks();
    Tween.stopAllByTarget(this.statusLabel.node);
    this.statusLabel.node.scale = StatusPointsText.DEFAULT_SCALE;
    ...
    const textAnimate: Function = () => {
      if (elapsedTime >= duration) {
        ...
        this.statusLabel.string = `${this.formatAmountString(Math.floor(this.targetAmount))} / ${this.formatAmountString(this.maxAmount)}`;

        Tween.stopAllByTarget(this.statusLabel.node);
        tween(this.statusLabel.node)
          .to(.15, { scale: StatusPointsText.DEFAULT_SCALE })
          .start();
        ...
        this.statusLabel.string = `${this.formatAmountString(Math.round(currentAmount))} / ${this.formatAmountString(this.maxAmount)}`;
        elapsedTime += .01;
